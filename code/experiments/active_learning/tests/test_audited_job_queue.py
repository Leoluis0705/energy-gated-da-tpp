...
def test_resume_accepts_combined_protocol_and_task_hash(tmp_path):
    protocol = tmp_path / "protocol.json"
    task = tmp_path / "task.json"
    protocol.write_bytes(b'{"protocol":"frozen"}')
    task.write_bytes(b'{"task":"mn"}')
    combined_hash = hashlib.sha256(protocol.read_bytes() + task.read_bytes()).hexdigest()

    completed = task_row(tmp_path, 2)
    completed["status"] = "FAILED"
    completed["failure_reason"] = "stale_running_process_missing"
    completed["config_hash"] = combined_hash
    ...
    (output / "run_config.json").write_text(
        json.dumps(
            {
                "formal_protocol_sha256": hashlib.sha256(protocol.read_bytes()).hexdigest(),
                "formal_protocol_path": str(protocol),
                "interval_task_path": str(task),
            }
        ),
        encoding="utf-8",
    )

    reconciled = reconcile_resume_state(pd.DataFrame([completed]), resource_kind="gpu")

    assert reconciled.loc[0, "status"] == "DONE"
    assert reconciled.loc[0, "sha256"]
    assert "reconciled_verified_done_output" in reconciled.loc[0, "failure_reason"]


def test_resume_rejects_wrong_combined_protocol_and_task_hash(tmp_path):
    protocol = tmp_path / "protocol.json"
    task = tmp_path / "task.json"
    protocol.write_bytes(b'{"protocol":"frozen"}')
    task.write_bytes(b'{"task":"mn"}')

    completed = task_row(tmp_path, 3)
    completed["status"] = "DONE"
    completed["config_hash"] = "0" * 64
    output = Path(completed["output_path"])
    output.mkdir()
    (output / "status.json").write_text(json.dumps({"status": "DONE"}), encoding="utf-8")
    (output / "run_config.json").write_text(
    ...
    with pytest.raises(QueueConfigurationError, match="DONE config hash mismatch"):
        reconcile_resume_state(pd.DataFrame([completed]), resource_kind="gpu")
