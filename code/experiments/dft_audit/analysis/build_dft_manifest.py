...
def build_pilot_artifact_search(root: Path) -> pd.DataFrame:
    ...
    remote_search = archive_root / "dft/audit/remote_pilot_search_20260717T193331Z"
    remote_metadata_path = remote_search / "search_metadata.json"
    remote_summary_path = remote_search / "candidate_summary.csv"
    remote_metadata = json.loads(remote_metadata_path.read_text(encoding="utf-8"))
    remote_summary = pd.read_csv(remote_summary_path)
    found = remote_summary["original_relaxation_outcar_or_oszicar_found"].map(
        lambda value: str(value).strip().lower() == "true"
    )
    if len(remote_summary) != 8 or found.any():
        raise ValueError("live pilot-artifact evidence does not support the declared result")
    bundles = [
    ...
    rows = [
        ...
        },
        {
            ...
            "search_method": "authenticated read-only recursive inventory; OUTCAR NSW classification; file size, mtime, and SHA-256 capture; POTCAR excluded",
            "result": (
                f"live search at {remote_metadata['checked_at_utc']} found retained relax.log files "
                "and only NSW=0 current OUTCAR/OSZICAR pairs; all eight original relaxation "
                f"artifacts remain unavailable (summary SHA-256 {sha256_file(remote_summary_path)})"
            ),
            "original_relaxation_outcar_or_oszicar_found": False,
        },
        {
...
def build_timeline_report(manifest: pd.DataFrame, artifact_search: pd.DataFrame) -> str:
    ...
    lines.extend(
        [
            ...
            _source_line(
                root / "dft/audit/remote_pilot_search_20260717T193331Z/candidate_summary.csv",
                root,
            ),
            _source_line(
                root / "dft/audit/remote_pilot_search_20260717T193331Z/evidence_files.csv",
                root,
            ),
            _source_line(
                root / "dft/audit/remote_pilot_search_20260717T193331Z/search_metadata.json",
                root,
            ),
            "",
        ]
